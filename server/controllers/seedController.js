...
// Mapping assets images to Unsplash URLs for the server
const blog_pic_1 = "https://images.unsplash.com/photo-1498050108023-c5249f4df085?ixlib=rb-4.0.3&auto=format&fit=crop&w=1472&q=80";
const blog_pic_2 = "https://images.unsplash.com/photo-1488590528505-98d2b5aba04b?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_3 = "https://images.unsplash.com/photo-1519389950473-47ba0277781c?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_4 = "https://images.unsplash.com/photo-1542831371-29b0f74f9713?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_5 = "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?ixlib=rb-4.0.3&auto=format&fit=crop&w=1471&q=80";
const blog_pic_6 = "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_7 = "https://images.unsplash.com/photo-1555099962-4199c345e5dd?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_8 = "https://images.unsplash.com/photo-1555421689-d68471e189f2?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_9 = "https://images.unsplash.com/photo-1523240795612-9a054b0db644?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
const blog_pic_10 = "https://images.unsplash.com/photo-1517048676732-d65bc937f952?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
...
export const seedDatabase = async (req, res) => {
    try {
        ...
        if (!authorId) {
            ...
            const newUser = await User.create({ name: "Admin", email: "admin@example.com", password: passwordHash, role: "admin" });
            authorId = newUser._id;
        }

        // Prepare Blogs with current user author
        const blogsToInsert = blog_data.map(blog => ({
            ...blog,
            author: authorId,
            status: "published" // Ensure they are published
        }));

        // Insert Blogs (ordered: false to skip duplicates/errors)
        try {
            await Blog.insertMany(blogsToInsert, { ordered: false });
            console.log("Blogs inserted (duplicates skipped)");
        } catch (e) {
            console.log("Some blogs might have been skipped (duplicates):", e.message);
        }

        // Prepare Comments
        // Note: Comment schema usually needs a 'user' field too. We'll assign the same author if not present.
        const commentsToInsert = comments_data.map(comment => ({
            ...comment,
            user: authorId // Assigning same user for simplicity as we don't have the original users
        }));

        try {
            await Comment.insertMany(commentsToInsert, { ordered: false });
            console.log("Comments inserted (duplicates skipped)");
        } catch (e) {
            console.log("Some comments might have been skipped:", e.message);
        }

        console.log("Seed Completed Successfully");
        res.json({ success: true, message: "Database Seeded Successfully! Refresh your dashboard." });
